...
INSTALL_SH_SUPPORTED_VERSION=v1.4.0
...
github_release_checksums_sig_url() (
  github_release_asset_url "$@" "sha256_checksums.txt.sig"
)
...
github_release_checksums_cert_url() (
  github_release_asset_url "$@" "sha256_checksums.txt.pem"
)
...
uname_arch() (
  arch=$(uname -m)
  case $arch in
    x86_64) arch="x86_64" ;; # goreleaser converted x86_64
    i86pc) arch="x86_64" ;;
    x86) arch="i386" ;; # goreleaser converted i386
    i686) arch="i386" ;;
    i386) arch="i386" ;;
    aarch64) arch="arm64" ;;
    armv5*) arch="armv5" ;;
...
verify_sign() {
  ...
  ${COSIGN_BINARY} \
    ...
    --certificate-identity "https://github.com/${OWNER}/${REPO}/.github/workflows/cd.yml@refs/tags/v${version}" \
...
download_asset() (
  ...
  if [ "$VERIFY_SIGN" = true ]; then
    ...
    if ! verify_sign "${checksums_filepath}" "${checksums_cert_file_url}" "${checksum_sig_file_url}" "${version}"; then
